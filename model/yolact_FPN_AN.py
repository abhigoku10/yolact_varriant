...
from data.config_attn import cfg, mask_type
from layers import Detect
from layers.interpolate import InterpolateModule
from backbone.backbone_yolact_PAMCAM import construct_backbone

import torch.backends.cudnn as cudnn
from utils import timer
from utils.functions import MovingAverage
from attention.attention import PAM_Module, CAM_Module
...
class FPN(ScriptModuleWrapper):
    ...
    def __init__(self, in_channels, norm_layer=nn.BatchNorm2d):
        ...
        ######################################################################################################
        self.conv5a = nn.Sequential(nn.Conv2d(256, 64, 3, padding=1, bias=False),
                                   norm_layer(64),
                                   nn.ReLU())
        
        self.conv5c = nn.Sequential(nn.Conv2d(256, 64, 3, padding=1, bias=False),
                                   norm_layer(64),
                                   nn.ReLU())

        self.sa = PAM_Module(64)
        self.sc = CAM_Module(64)

        self.conv51 = nn.Sequential(nn.Conv2d(64, 64, 3, padding=1, bias=False),
                                   norm_layer(64),
                                   nn.ReLU())
        self.conv52 = nn.Sequential(nn.Conv2d(64, 64, 3, padding=1, bias=False),
                                   norm_layer(64),
                                   nn.ReLU())

        self.conv6 = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(64, 256, 1))
        self.conv7 = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(64, 256, 1))

        self.conv8 = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(64, 256, 1))
        ######################################################################################################
    ...
    def forward(self, convouts:List[torch.Tensor]):
        ...
        for pred_layer in self.pred_layers:
            ...
            # if j==len(convouts):
            ##############################################
            feat1 = self.conv5a(out[j])
            sa_feat = self.sa(feat1)
            sa_conv = self.conv51(sa_feat)
            sa_output = self.conv6(sa_conv)

            feat2 = self.conv5c(out[j])
            ...
            out[j] = self.conv8(feat_sum)

            ##############################################
        # out[1] = F.relu(pred_layer(out[1]))
        # out[0] = F.relu(pred_layer(out[0]))
...
class Yolact(nn.Module):
    ...
    def __init__(self):
        ...
        # for i in self.backbone.state_dict():
        #     print(i,self.backbone.state_dict()[i].shape)
        ...
        # print(self.backbone.state_dict()[self.selected_layers[0]].shape)
    ...
    def load_weights(self, path):
        ...
        #Added for increamental training
        try:
            self.load_state_dict(state_dict)
    ...
    def forward(self, x):
        ...
        # print("yolact x size",x.size())

        with timer.env('backbone'):
            outs = self.backbone(x)
            # print('len',outs[0].shape,outs[1].shape)

        if cfg.fpn is not None:
            with timer.env('fpn'):
                # Use backbone.selected_layers because we overwrote self.selected_layers
                outs = [outs[i] for i in cfg.backbone.selected_layers]
                # for i in cfg.backbone.selected_layers:
                #     print('OUTS',i)
                #     print(outs[i].shape)
                outs = self.fpn(outs)
