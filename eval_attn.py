...
from model.yolact_FPN_AN import Yolact
...
from config import cfg_yolact

# daytime_classes = {'dawnordusk': 0, 'night': 1, 'daytime': 2, 'undefined': 3}
daytime_classes = ['dawn_or_dusk', 'night', 'daytime', 'undefined']
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45):
    ...
    if args.display_masks and cfg.eval_mask_branch:
        ...
        for i in range(0,masks.shape[0]):
            # print("masks[i].shape",(masks[i].shape))
            mask_temp = masks[i].cpu().numpy()

            # print("mask type",type(mask_temp))
            # print(np.where(mask_temp != 0.0))
            mask_temp = mask_temp.squeeze()
            # for row in range((mask_temp.shape[0])):
            #     for column in range((mask_temp.shape[1])):
            #         if mask_temp[row][column] != 0.0:
            #             print(mask_temp[row][column])
            mask_list.append(mask_temp)
            polygons = Mask(mask_temp).polygons()
        ...
        for index in range(1,len(mask_list)):
            #print(type(mask_list[index]),mask_list[index].shape)
            row,columns = np.where(mask_list[index]!=0.0)
            zipped = list(zip(row,columns))
            for values in zipped:
                mask_list[0][values[0]][values[1]] = mask_list[index][values[0]][values[1]]
            # for row in range((mask_list[0].shape[0])):
            #     for column in range((mask_list[0].shape[1])):
            #         if mask_list[index][row][column] != 0.0:
            #             mask_list[0][row][column] = mask_list[index][row][column]
            # # imageio.imwrite(str(index)+'eg.jpg',mask_list[0])
        imageio.imwrite('sample.png',mask_list[0])
...
def prep_metrics(ap_data, dets, img, gt, gt_masks, h, w, num_crowd, image_id, detections:Detections=None):
    ...
    with timer.env('Eval Setup'):
        ...
        print("numpred",num_pred," num_gt",num_gt)
...
def evalimage(net:Yolact, path:str, save_path:str=None):
    ...
    #preds, daytime_prob = net(batch)
    preds = net(batch)
    
    # print('===================>daytime_probability:', daytime_prob)
    ...
    #print('Printing classification label!!!!!!!!!!!!!------------------------>>>>>>>', daytime_info)
    ...
    if save_path is None:
        ...
        plt.show()
    #else:
    #    cv2.putText(img_numpy, daytime_info, (100, 80), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (255, 255, 0), 1, cv2.LINE_AA)
        ...
        for iou_idx in range(len(iou_thresholds)):
            for iou_type in ('box', 'mask'):
                ...
                if not ap_obj.is_empty():
                    # print('ap obj',ap_obj.num_gt_positives)
                    aps[iou_idx][iou_type].append(ap_obj.get_ap())
        print(cfg.dataset.class_names[_class])

        all_maps_value = {'box': {}, 'mask': {}}
        all_maps = {'box':OrderedDict(all_maps_value['box']),'mask':OrderedDict(all_maps_value['mask'])}
        for iou_type in (['box', 'mask']):
            for i, threshold in enumerate(iou_thresholds):
                if len(aps[i][iou_type]) > 0:
                    # print(aps[i][iou_type])
                    all_maps[iou_type][int(threshold*100)] = aps[i][iou_type][0]
                else:
                    all_maps[iou_type][int(threshold*100)] = None

            # print(all_maps[iou_type].values(),len(all_maps[iou_type].values()))
            all_maps[iou_type]['all'] = (sum(filter(None,all_maps[iou_type].values())) / (len(all_maps[iou_type].values())-1))
        # print(all_maps)
        print_maps(all_maps)

        # for iou_type in (['box', 'mask']):
        #   for x in aps[iou_type]:
        #       print(' %5s |' * len(aps[iou_type]) %  str(x))
        #   print('-------+' * len(aps[iou_type]))
...
def print_maps(all_maps):
    # Warning: hacky 
    make_row_head = lambda vals: (' %8s |' * len(vals)) % tuple(vals)
    make_row = lambda vals: (' %8s |' * len(vals)) % tuple(vals)
    make_sep = lambda n:  ('------------' * n)

    print()
    print(make_row_head([''] + [('.%d ' % x if isinstance(x, int) else x + ' ') for x in all_maps['box'].keys()]))
    print(make_sep(len(all_maps['box']) ))
    for iou_type in ('box', 'mask'):
        # print(all_maps[iou_type].values())
        print(make_row([iou_type] + [ '%.4f' % x  if isinstance(x, float) else str('') for x in all_maps[iou_type].values()]))
    print(make_sep(len(all_maps['box']) ))
    print()
