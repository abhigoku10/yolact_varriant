...
class Detect(object):
    ...
    def detect(self, batch_idx, conf_preds, decoded_boxes, mask_data, inst_data):
        ...
        #print("kept boxes",boxes[0])
      #   for box in boxes:
      #   	box[:, 0], box[:, 2] = sanitize_coordinates(box[:, 0], box[:, 2], b_w, cast=False)
		    # box[:, 1], box[:, 3] = sanitize_coordinates(box[:, 1], box[:, 3], b_h, cast=False)
		    # box = box.long()
