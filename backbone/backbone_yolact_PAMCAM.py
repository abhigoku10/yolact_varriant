...
class ResNetBackbone(nn.Module):
    ...
    def __init__(self, layers, atrous_layers=[], block=Bottleneck,  dilated=True, norm_layer=nn.BatchNorm2d, multi_grid=False, multi_dilation=None):
        super().__init__()
        ...
        self.conv5c = nn.Sequential(nn.Conv2d(2048, 512, 3, padding=1, bias=False),
                                   norm_layer(512),
                                   nn.ReLU())
        ...
        self.conv51 = nn.Sequential(nn.Conv2d(512, 512, 3, padding=1, bias=False),
                                   norm_layer(512),
                                   nn.ReLU())
        self.conv52 = nn.Sequential(nn.Conv2d(512, 512, 3, padding=1, bias=False),
                                   norm_layer(512),
                                   nn.ReLU())

        self.conv6 = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(512, 2048, 1))
        self.conv7 = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(512, 2048, 1))

        self.conv8 = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(512, 2048, 1))
        ######################################################################################################
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, norm_layer=None, multi_grid=False, multi_dilation=None):
        ...
        if multi_grid == False:
            if dilation == 1 or dilation == 2:
                layers.append(block(self.inplanes, planes, stride, downsample, self.norm_layer, dilation=1))
            elif dilation == 4:
                layers.append(block(self.inplanes, planes, stride, downsample, self.norm_layer, dilation=2))
            else:
                raise RuntimeError("=> unknown dilation size: {}".format(dilation))
        else:
            layers.append(block(self.inplanes, planes, stride, downsample, self.norm_layer, dilation=multi_dilation[0]))
        ...
        if multi_grid:
            ...
            for i in range(1, blocks):
                layers.append(block(self.inplanes, planes, previous_dilation=dilation, norm_layer=self.norm_layer, dilation=multi_dilation[i%div]))
        else:
            for i in range(1, blocks):
                layers.append(block(self.inplanes, planes, dilation=dilation, norm_layer=self.norm_layer))
    ...
    def forward(self, x):
        ...
        # print('---------------layer 4 output', x)
        # sa = PAM_Module(16)
        outs.append(sasc_output)
